...
class Tester:
    ...
    def execute_and_retention(self, seed_pool: SeedPool):
        ...
        # # code for colllect operation/dialect coverage
        # global OPERATION_COV, DIALECT_COV
        # if args.collect_dialect_coverage:
        #     c = Coverage(self.seed.path)
        ...
        for option_idx in range(len(selected_options)):
            ...
            cmd = ' '.join([args.mlir_opt, option, self.seed.path, '>', out_mlir, '2>', report])
...
def main():
    ...
    while not seed_pool.empty() and test_time <= args.fuzz_time:
        programs_to_collect = []
        ...
        if len(diff(new_seed.path, old_seed.path)) > 10:
            ...
            if c1.collect_and_compare():
                ...
                if not new_seed.tested:
                    ...
                    t.execute_and_retention(seed_pool)
                    programs_to_collect.append(new_seed)
            else:
        ...
        # operation and dialect coverage collect for per program
        if args.collect_dialect_coverage:
            for seed in programs_to_collect:
                c = Coverage(seed.path)
                operations = list(c.collect_cov(0))
                operations = {_.split('(')[0] for _ in operations}
                dialects = {_.split('.')[0] for _ in operations}
                OPERATION_COV = OPERATION_COV | operations
        ...
        # update operation and dialct coverage
...
--replace_data_edge /data3/2025.04.07.liangjingjing/bin/llvm-13c6abfac84fca4bc55c0721d1853ce86a385678/build/bin/ReplaceDataEdge \
--replace_control_edge /data3/2025.04.07.liangjingjing/bin/llvm-13c6abfac84fca4bc55c0721d1853ce86a385678/build/bin/ReplaceControlEdge \
--delete_node /data3/2025.04.07.liangjingjing/bin/llvm-13c6abfac84fca4bc55c0721d1853ce86a385678/build/bin/DeleteNode \
--create_node /data3/2025.04.07.liangjingjing/bin/llvm-13c6abfac84fca4bc55c0721d1853ce86a385678/build/bin/CreateNode \
--mlir_opt /data3/2025.04.07.liangjingjing/bin/llvm-13c6abfac84fca4bc55c0721d1853ce86a385678/build/bin/mlir-opt \
--optfile opt.txt \
...
if __name__ == '__main__':
    ...
    parser.add_argument("--collect_dialect_coverage", type=bool, help="Whether collect dialect and operation coverage per-hour.", default=True)
    parser.add_argument("--dialect_coverage_dir", type=str, help="Dialect coverage directory.", default="dialect_cov")
    parser.add_argument("--operation_coverage_dir", type=str, help="Operation coverage directory.", default="operation_cov")
    parser.add_argument("--collect_interval", type=int, help="The time interval for dialet/operation coverage collection.", default=60*60)
    ...
    if args.clean:
        os.system(f'rm -rf {args.seed_dir} {args.report_dir} {args.dialect_coverage_dir}')
