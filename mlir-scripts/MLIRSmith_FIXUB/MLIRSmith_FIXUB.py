...
def rdm_opt(opts: list, prob: float):
    """
    Random select optimizations with probability.

    Params:
    opts: list, candidate optimizations
    prob: float, probability for selecting each optimization

    Return:
        randomly selected optimizations.
    """
    num = math.ceil(len(opts)*prob)
    return random.sample(opts, num)
...
def main():
    mlirsmith_factory = MLIRSmithSeedFactory()
    prog_num = 0
    while prog_num < 10000:
        s = mlirsmith_factory.gen_seed()
        # s.path = "/data3/tmp/FIXUB_MLIRSmith/seeds1/tmp.Y1gNYdiGhA.mlir"
        test_batch = TestBatch(s.path)
        res = test_batch.fix_and_lowering()
        print(f"[main] test result: {[test_batch.test_dir, res]}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--init_size", type=int, help="Size of seed pool.", default=5000)
    ...
    # pass file params
    parser.add_argument("--opt_passes_file", type=str, help="Path of optimization passes file", default="fuzzingopts/optpasses.txt")
    ...
    parser.add_argument("--prerequisite_passes_file", type=str, help="Path of prerequisite passes file", default="prerequisitepasses.txt")
    ...
    parser.add_argument("--timeout", type=int, help="time limit for each step.", default=10)
    parser.add_argument("--clean", type=bool, help="Clean", default=False)
    # arguments in coverage collection.
    parser.add_argument("--fuzzing_time", type=int, help="The fuzzing time", default=86400)
    parser.add_argument("--collect_step", type=int, help="The time step for each coveration collection", default=3600)
    ...
    assert os.path.exists(args.mlirsmith)
