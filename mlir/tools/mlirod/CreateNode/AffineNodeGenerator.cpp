...
OpGenerator affineApplyGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value ret =
        mlir::affine::AffineApplyOp::create(builder, loc, map, operands);
    return true;
  };
}
...
OpGenerator affineForGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    // Strange logic in MLIRSmith, why argument and return value decide
    // together?
    ...
    auto blockBuilder = [&](OpBuilder &b, Location loc,
      ...
      if (hasRetVal) {
        ...
        if (mb->valuePool.pool.count(ty)) {
          ...
          mlir::affine::AffineYieldOp::create(builder, loc, ret);
        } else {
          mlir::affine::AffineYieldOp::create(builder, loc);
        }
      } else {
        mlir::affine::AffineYieldOp::create(builder, loc);
      }
    ...
    if (hasIterArg) {
      auto res = mlir::affine::AffineForOp::create(
          builder, loc, lowerBound, upperBound, step,
          llvm::ArrayRef(iterArgVal), blockBuilder);
    } else {
      auto value = mlir::affine::AffineForOp::create(
          builder, loc, lowerBound, upperBound, step, mlir::ValueRange(),
          blockBuilder);
    }
    return true;
  };
...
OpGenerator affineIfGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (candidates.size() == 0) {
      ...
      auto integerTy = mlir::dyn_cast<IntegerType>(
          supportedTys[rollIdx(supportedTys.size())]);
    ...
    llvm::outs() << "[affineIfGenerator] length of resultTypes: "
                 << std::to_string(resultTypes.size()) << "\n";

    auto ifOp = mlir::affine::AffineIfOp::create(builder, loc, resultTypes, set,
                                                 args, withElseRegion);
      ...
      if (hasResult) {
        ...
        if (childMb->valuePool.pool.count(resTyStr)) {
          candidates.insert(candidates.end(),
                            childMb->valuePool.pool[resTyStr].begin(),
                            childMb->valuePool.pool[resTyStr].end());
        }
        if (candidates.empty()) {
          candidates.push_back(
              generateTypedValue(builder, loc, resultType, childMb));
        }
        auto ret = candidates[rollIdx(candidates.size())];
        mlir::affine::AffineYieldOp::create(builder, loc, ValueRange(ret));
      }
      delete childMb;
    };
    // Then block
...
OpGenerator affineLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    if (memCandidates.empty())
      memCandidates.push_back(generateRankedMemref(
    ...
    // Generate operation
    assert(memref);
    mlir::Value res = mlir::affine::AffineLoadOp::create(builder, loc, memref,
                                                         ValueRange(indices));
    return true;
  };
}
...
OpGenerator affineStoreGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    for (uint32_t i = 0; i < dim; ++i)
      indices.push_back(idxCandidates[rollIdx(idxCandidates.size())]);
    mlir::affine::AffineStoreOp::create(builder, loc, elem2Store, memref,
                                        ValueRange(indices));
    return true;
  };
}
...
OpGenerator affineMaxGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    auto index =
        mlir::affine::AffineMaxOp::create(builder, loc, map, mapOperands);
...
OpGenerator affineMinGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    for (int i = 0; i < dimCount + symbolCount; ++i)
      mapOperands.push_back(candidates[rollIdx(candidates.size())]);
    auto index =
        mlir::affine::AffineMinOp::create(builder, loc, map, mapOperands);
    // if (index->getResultTypes().size() == 1 &&
    // index->getResultTypes()[0].dyn_cast<IndexType>())
    //   parentMb->add2Pool(index->getResult(0));
    return true;
...
OpGenerator affineParallelGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    auto op = mlir::affine::AffineParallelOp::create(builder, loc, resultTypes,
                                                     reductions, ranges);
    ...
    builder.setInsertionPointToStart(&block);
    {
      ...
      for (unsigned stIdx = 0; stIdx < statementNum; ++stIdx) {
        ...
        llvm::outs() << "[affineParallelGenerator] Selected generator: "
                     << generatorsStr[idx] << "\n";
      ...
      if (!resultTypes.empty()) {
        ...
        for (auto resultTy : resultTypes) {
          ...
          if (childMb->valuePool.pool.count(resultTyStr)) {
            retsCandidates.insert(retsCandidates.end(),
                                  childMb->valuePool.pool[resultTyStr].begin(),
                                  childMb->valuePool.pool[resultTyStr].end());
          }
          if (retsCandidates.empty()) {
            retsCandidates.push_back(
                generateTypedValue(builder, loc, resultTy, childMb));
          }
          rets.push_back(retsCandidates[rollIdx(retsCandidates.size())]);
        }
        mlir::affine::AffineYieldOp::create(builder, loc, ValueRange(rets));
      }
      delete childMb;
    }
    builder.restoreInsertionPoint(point);
...
OpGenerator affinePrefetchGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    if (memCandidates.empty())
      memCandidates.push_back(generateStaticShapedMemref(
          builder, loc, randomStaticShapedMemrefType(builder.getContext())));
    auto memref = memCandidates[rollIdx(memCandidates.size())];
    SmallVector<Value> indices;
    mlir::affine::AffinePrefetchOp::create(builder, loc, memref, indices,
                                           rollIdx(2), rollIdx(4), rollIdx(2),
                                           randomAffineMap(builder));
    return true;
  };
}
...
OpGenerator affineVectorLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    if (idxCandidates.empty())
      return false;
    SmallVector<Value> indices;
    auto memTy = mlir::dyn_cast<MemRefType>(memref.getType());
    for (int i = 0; i < memTy.getRank(); ++i)
      indices.push_back(idxCandidates[rollIdx(idxCandidates.size())]);
    auto vecTy = random1DVectorType(memTy.getElementType());
    mlir::Value op = mlir::affine::AffineVectorLoadOp::create(
        builder, loc, vecTy, memref, ValueRange(indices));
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    parentMb->search(
        [](std::string s, mlir::Value v) -> bool {
          ...
          if (!cond1)
            return false;
          if (auto ty = mlir::dyn_cast<VectorType>(v.getType())) {
            return ty.getRank() == 1;
          }
          return false;
        },
    ...
    if (vecCandidates.empty())
      vecCandidates.push_back(generateVector(
          builder, loc, random1DVectorType(builder.getContext()), parentMb));
    auto vec2Store = vecCandidates[rollIdx(vecCandidates.size())];
    auto vecTy = mlir::dyn_cast<VectorType>(vec2Store.getType());
    ...
    parentMb->search(
        [&](std::string s, mlir::Value v) -> bool {
          ...
          bool cond2 = false;
          if (auto rankedMemRefType = mlir::dyn_cast<MemRefType>(valueType))
            cond2 = mlir::dyn_cast<ShapedType>(v.getType()).getElementType() ==
    ...
    auto op = mlir::affine::AffineVectorStoreOp::create(
        builder, loc, vec2Store, memref, ValueRange(indices));
    return true;
  };
}