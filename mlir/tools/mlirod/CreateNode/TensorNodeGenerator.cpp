...
OpGenerator tensorCastGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::CastOp::create(builder, loc, destTy, src);
    mb->add2Pool(res);
    return true;
  };
}
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mb->search(
        [](std::string s, mlir::Value v) -> bool {
          ...
          return mlir::dyn_cast<TensorType>(v.getType()).getRank() >= 2;
        },
        &candidates);
    if (candidates.empty()) {
      ...
      candidates.push_back(generateStaticShapedTensor(
          builder, loc, RankedTensorType::get(shape, type)));
    }
    auto source = candidates[rollIdx(candidates.size())];
    auto srcTy = mlir::dyn_cast<RankedTensorType>(source.getType());
    ...
    mlir::Value res =
        tensor::CollapseShapeOp::create(builder, loc, source, reIdxes);
...
OpGenerator tensorDimGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mb->search(
        ...
        &candidates);
    if (candidates.empty()) {
      candidates.push_back(generateRankedTensor(
          builder, loc, randomRankedTensorType(builder.getContext()), mb));
    }
    auto source = candidates[rollIdx(candidates.size())];
    auto srcTy = mlir::dyn_cast<RankedTensorType>(source.getType());
    ...
    mlir::Value res = tensor::DimOp::create(builder, loc, source, idx);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator tensorEmptyGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    if (rollIdx(2)) {
      ...
      mlir::Value res = tensor::EmptyOp::create(
          builder, loc, tensorTy.getShape(), tensorTy.getElementType());
      mb->add2Pool(res);
      return true;
    } else {
      auto shape = getRandomShape();
      SmallVector<Value> dynamicSizes;
      std::vector<mlir::Value> dynamicSizesCandidates =
          std::vector<mlir::Value>();
      mb->search(index_filter, &dynamicSizesCandidates);
      if (dynamicSizesCandidates.empty()) {
        dynamicSizesCandidates.push_back(
            generateIndex(builder, loc, rollIdx(100)));
      }
      for (size_t i = 0; i < shape.size(); ++i) {
        if (rollIdx(2)) {
          shape[i] = ShapedType::kDynamic;
          dynamicSizes.push_back(
              dynamicSizesCandidates[rollIdx(dynamicSizesCandidates.size())]);
        }
      }
      auto tensorTy = RankedTensorType::get(
          shape, randomIntOrFloatType(builder.getContext()));
      tensor::EmptyOp::create(builder, loc, tensorTy, ValueRange(dynamicSizes));
    }
    return true;
  };
}
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mb->search(
        [](std::string s, mlir::Value v) -> bool {
          ...
          auto tensorTy = mlir::dyn_cast<TensorType>(v.getType());
    ...
    // mlir::Value res = tensor::ExpandShapeOp::create(builder,
    // 	loc, resTy, source, reassociation, SmallVector<NamedAttribute>());
    mlir::Value res = tensor::ExpandShapeOp::create(builder, loc, resTy, source,
                                                    reassociation);
    mb->add2Pool(res);
    return true;
  };
...
OpGenerator tensorExtractGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto source = candidates[rollIdx(candidates.size())];
    auto srcTy = mlir::dyn_cast<ShapedType>(source.getType());
    auto indices = randomIndicesForShapedType(srcTy, builder, loc);
    mlir::Value res = tensor::ExtractOp::create(builder, loc, source, indices);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator tensorExtractSliceGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (srcCandidates.empty()) {
      srcCandidates.push_back(generateRankedTensor(
          builder, loc, randomRankedTensorType(builder.getContext()), mb));
    }
    ...
    tensor::ExtractSliceOp::create(builder, loc, destTy, src, offsets, sizes,
                                   strides);
    return true;
  };
}
...
OpGenerator tensorFromElementsGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::FromElementsOp::create(builder, loc, tensorTy,
                                                     ValueRange(elements));
    mb->add2Pool(res);
    return true;
  };
...
OpGenerator tensorGenerateGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::GenerateOp::create(
        ...
        [&](OpBuilder &b, Location loc, ValueRange args) {
          ...
          if (childMb->valuePool.pool.count(dynTenTyStr)) {
            retCandidates.insert(retCandidates.end(),
          ...
          if (retCandidates.empty()) {
            retCandidates.push_back(
                generateElement(builder, loc, dynTenTy.getElementType()));
          }
          auto ret = retCandidates[rollIdx(retCandidates.size())];
          tensor::YieldOp::create(builder, loc, ret);
        });
...
OpGenerator tensorInsertGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (mb->valuePool.pool.count(elemTyStr)) {
      elemCandidates.insert(elemCandidates.begin(),
    ...
    auto indices = randomIndicesForShapedType(srcTy, builder, loc);
    tensor::InsertOp::create(builder, loc, elem, source, indices);
    return true;
  };
}
...
OpGenerator tensorInsertSliceGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (destCandidates.empty()) {
      destCandidates.push_back(generateRankedTensor(
          builder, loc, randomRankedTensorType(builder.getContext()), mb));
    }
    auto dest = destCandidates[rollIdx(destCandidates.size())];
    auto destTy = mlir::dyn_cast<RankedTensorType>(dest.getType());
    ...
    auto src = srcCandidates[rollIdx(srcCandidates.size())];
    tensor::InsertSliceOp::create(builder, loc, src, dest, offsets, sizes,
                                  strides);
    return true;
  };
}
...
OpGenerator tensorPackGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto srcTy = mlir::dyn_cast<TensorType>(src.getType());
    ...
    if (padCandidates.empty()) {
      padCandidates.push_back(
          generateElement(builder, loc, srcTy.getElementType()));
    }
    ...
    // tensor::PackOp::create(builder, loc, src, dest, innerDimsPos, innerTiles,
    //                        std::optional<Value>(pad), outerDimsPos);
    mlir::linalg::PackOp::create(builder, loc, src, dest, innerDimsPos,
                                 innerTiles, std::optional<Value>(pad),
                                 outerDimsPos)
        .getOperation();
    return true;
  };
...
OpGenerator tensorRankGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto source = candidates[rollIdx(candidates.size())];
    mlir::Value res = tensor::RankOp::create(builder, loc, source);
    mb->add2Pool(res);
    return true;
  };
}
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (indicesCandidates.empty()) {
      indicesCandidates.push_back(
          generateStaticShapedTensor(builder, loc, indicesTy));
    }
    auto indices = indicesCandidates[rollIdx(indicesCandidates.size())];

    mlir::Value res = tensor::ScatterOp::create(builder, loc, destTy, src, dest,
                                                indices, scatterDims, true);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator tensorSplatGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::SplatOp::create(builder, loc, elem, tensorTy);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator tensorUnpackGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (mb->valuePool.pool.count(destTyStr)) {
      destCandidates.insert(destCandidates.begin(),
                            mb->valuePool.pool[destTyStr].begin(),
                            mb->valuePool.pool[destTyStr].end());
    }
    if (destCandidates.empty()) {
      destCandidates.push_back(
          generateStaticShapedTensor(builder, loc, destTy));
    }
    auto dest = destCandidates[rollIdx(destCandidates.size())];
    // tensor::UnPackOp::create(builder, loc, source, dest, innerDimsPos,
    //                                  innerTiles, outerDimsPerm);
    mlir::linalg::UnPackOp::create(builder, loc, source, dest, innerDimsPos,
                                   innerTiles, outerDimsPerm)
        .getOperation();
    return true;
  };
