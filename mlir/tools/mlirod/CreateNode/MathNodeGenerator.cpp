		...
		mb->search(
			[] (std::string s, mlir::Value v) -> bool {
				...
				return mlir::isa<FloatType>(v.getType()) || 
					(mlir::isa<ShapedType>(v.getType()) && 
					mlir::isa<FloatType>(mlir::dyn_cast<ShapedType>(v.getType()).getElementType()));
			},
			&operandCandidates
		);
    ...
    std::vector<mlir::Value> operandCandidates = std::vector<mlir::Value>();
		mb->search(
			[] (std::string s, mlir::Value v) -> bool {
				bool cond1 = int_float_filter(s) || ranked_tensor_filter(s, v);
				if (!cond1) { return false; }
				return mlir::isa<IntegerType>(v.getType()) || 
					(mlir::isa<ShapedType>(v.getType()) && 
					mlir::isa<IntegerType>(mlir::dyn_cast<ShapedType>(v.getType()).getElementType()));
			},
			&operandCandidates
		);
		if (operandCandidates.empty()) {
...
OpGenerator fpowiGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock* mb) -> bool {
    ...
    std::vector<mlir::Value> candidates2;
		if (mlir::dyn_cast<FloatType>(operand0.getType())) {
      ...
      auto t = RankedTensorType::get(mlir::dyn_cast<ShapedType>(operand0.getType()).getShape(), i32);
      ...
      Type ty;
      if (mlir::isa<FloatType>(operand0.getType())) {
        ty = builder.getI32Type();
      } else {
        ty = RankedTensorType::get(mlir::dyn_cast<ShapedType>(operand0.getType()).getShape(), i32);
      }
