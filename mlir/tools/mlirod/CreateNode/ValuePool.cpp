...
bool ranked_memref_filter(std::string s, mlir::Value v) {
  ...
  bool cond2 = false;
  if (auto rankedMemRefType = mlir::dyn_cast<mlir::MemRefType>(valueType)) {
    cond2 = true;
  }
  return cond1 && cond2;
}
...
bool ranked_tensor_filter(std::string s, mlir::Value v) {
  ...
  mlir::Type valueType = v.getType();
  bool cond2 = false;
  if (auto rankedTensorType = mlir::dyn_cast<mlir::TensorType>(valueType)) {
...
bool ranked_memref_tensor_filter_has_dim(std::string s, mlir::Value v) {
  ...
  bool cond2 = mlir::dyn_cast<ShapedType>(v.getType()).getRank() > 0;
  return cond2;
}
