      ...
      resultTypes.push_back(randomNonTensorType(builder.getContext()));
        ...
        auto retTy = resultTypes[0];
          ...
          retCandidates.push_back(generateTypedValue(builder, loc, retTy, childMb));
				...
				scf::YieldOp::create(builder, loc, ret);
      } else {
		...
		scf::IfOp ifOp;
    if (hasElse) {
      ifOp = scf::IfOp::create(builder, loc, condition, blockBuilder, blockBuilder);
    } else {
      ifOp = scf::IfOp::create(builder, loc, condition, blockBuilder);
    }
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock* mb) -> bool {
    ...
    for (int i = 0; i < caseRegionCount; ++i) {
      cases.push_back(i + 1);
		...
		auto idxSwitchOp = scf::IndexSwitchOp::create(builder, 
...
OpGenerator scfWhileGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock* mb) -> bool {
    ...
    auto condBuilder = [&](OpBuilder &b, Location loc, ValueRange args) {
      ...
      scf::ConditionOp::create(builder, loc, cond, ValueRange(condOperand));
    };
    scf::WhileOp::create(builder, 
    ...
    auto bodyBuilder = [&](OpBuilder &b, Location loc, ValueRange ivs, ValueRange args) {
      ...
      if (hasOperand) {
        ...
        scf::ReduceOp::create(b, 
            ...
            //   }
            //   mlir::Value reduceRet = reduceReturnCandidates[rollIdx(reduceReturnCandidates.size())];
            //   // llvm::SmallVector<mlir::Value> ret = llvm::SmallVector<mlir::Value>();
            //   // ret.push_back(reduceRet);
            //   scf::ReduceReturnOp::create(builder1, loc, reduceRet);
            // }
          );
      }
    };
