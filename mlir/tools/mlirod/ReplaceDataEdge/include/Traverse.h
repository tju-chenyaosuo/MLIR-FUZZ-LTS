...
#ifdef DEBUG
  llvm::outs() << "[MutationBlock::add2Pool] add info: " << v.getType() << ", "
               << v.getLoc() << "\n";
#endif
...
#ifdef DEBUG
void MutationBlock::print() { valuePool.print(); }
#endif
...
struct MutationParser {
  ...
  MutationParser(double operandProb, unsigned mutPos)
      : operandProb(operandProb), mutPos(mutPos) {
    mutationCnt = 0;
    curPos = 0;
  }

  void printOperation(Operation *op, MutationBlock *mb);
  void getAllUsers(Operation *op, std::vector<mlir::Operation *> *allUsers);
};

void MutationParser::getAllUsers(Operation *op,
                                 std::vector<mlir::Operation *> *allUsers) {
...
void MutationParser::printOperation(Operation *op, MutationBlock *mb) {
  ...
  // Mutator
  // TODO: This is a quick fix for invalid mutation, please replace this logic
  // with official verify logic.
  std::vector<std::string> invalidParent = {"linalg.matmul", "linalg.dot"};
  std::vector<std::string> invalidOpts = {
      "affine.store",        "tensor.unpack",      "affine.load",
      "affine.vector_store", "affine.vector_load", "affine.if",
      "linalg.dot",          "vector.yield"};
  std::vector<std::string> invalidUsers = {"affine.vector_store",
                                           "affine.vector_load"};
  ...
  if (isValidOperation) {
    std::vector<mlir::Operation *> allUsers = std::vector<mlir::Operation *>();
    getAllUsers(op, &allUsers);
    for (auto user : allUsers) {
      std::string userOpName = getOperationName(user);
      for (int idx = 0; idx < invalidUsers.size(); ++idx) {
        if (userOpName == invalidUsers[idx]) {
          isValidOperation = false;
          break;
        }
      }
