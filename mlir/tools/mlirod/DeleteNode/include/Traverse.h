...
mlir::Value MutationValuePool::searchFilter(std::string ty,
...
mlir::Value MutationBlock::searchFilter(std::string ty,
                                        std::string operationId) {
...
struct MutationParser {
  ...
  MutationParser(unsigned mutPos) : mutPos(mutPos) {
    mutationCnt = 0;
    curPos = 0;
  }
...
void getDerectDependencies(mlir::Operation *op,
...
void changeUsage(mlir::Operation *op, std::vector<mlir::Operation *> *users,
                 MutationBlock *mb) {
  std::string opId = getOperationName(op) + "-" + getLocationStr(op);
  ...
  for (mlir::Operation *u : *users) {
    ...
    for (int oIdx = 0; oIdx < u->getNumOperands(); oIdx++) {
      ...
      std::string oldOpId = getOperationName(oldOperandOperation) + "-" +
                            getLocationStr(oldOperandOperation);
...
void MutationParser::printOperation(Operation *op, MutationBlock *mb) {
#ifdef DEBUG
  llvm::outs() << "[MutationParser::printOperation] traverse: "
               << getOperationName(op) << "-" << getLocationStr(op) << "\n";
#endif
  ...
  std::vector<std::string> invalidOpts = {
      "builtin.module", "func.func",     "scf.yield",
      "linalg.yield",   "vector.yield",  "scf.reduce.return",
      "affine.yield",   "scf.condition", "memref.alloca_scope.return",
      "scf.reduce",     "tensor.yield",  "vector.multi_reduction",
      "func.return",    "return"};
...
#ifdef DEBUG
        llvm::outs() << "[MutationParser::printOperation] Selected Operation: "
                     << getOperationName(op) << "-" << getLocationStr(op)
                     << "\n";
        for (mlir::Operation *o : dependencies)
          llvm::outs() << "[MutationParser::printOperation] "
                       << getOperationName(o) << "-" << getLocationStr(o)
                       << "\n";
#endif
