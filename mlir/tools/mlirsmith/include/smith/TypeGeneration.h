...
inline MemRefType littleOrEqualRankMemrefType(Type ty) {
  mlir::ShapedType shapedTy = mlir::dyn_cast<mlir::ShapedType>(ty);
  std::vector<int64_t> shape;
  int idx = 0;
  while (idx < shapedTy.getRank()) {
    // llvm::outs() << "shapedTy.getDimSize(idx)=" << shapedTy.getDimSize(idx)
    //              << "\n";
    if (shapedTy.getDimSize(idx) < 0) {
      shape.push_back(ShapedType::kDynamic);
    } else {
      shape.push_back(random(shapedTy.getDimSize(idx) - 1) + 1);
    }
    idx++;
  }
  return MemRefType::get(shape, shapedTy.getElementType());
}

inline VectorType littleOrEqualRankVectorType(Type ty) {

  mlir::ShapedType shapedTy = mlir::dyn_cast<mlir::ShapedType>(ty);
  std::vector<int64_t> shape;
  int idx = 0;
  while (idx < shapedTy.getRank()) {
    // llvm::outs() << "[littleOrEqualRankVectorType] idx=" << idx << ", shapedTy.getRank()=" << shapedTy.getRank() << "\n";
    // llvm::outs() << "shapedTy.getDimSize(idx)=" << shapedTy.getDimSize(idx)
    //              << "\n";
    if (shapedTy.getDimSize(idx) <= 1) {
      /**
       * This is a trade-off because vector has no dynamic dimension.
       * When the orignal type has dynamic dimension, we set the corresponding
       * dimension of shape to be 1, to make it valid.
       */
      shape.push_back(1);
    } else {
      shape.push_back(random(shapedTy.getDimSize(idx) - 1) + 1);
  ...
  // llvm::outs() << "[littleOrEqualRankVectorType] debug point\n";
  return VectorType::get(shape, shapedTy.getElementType());
}
