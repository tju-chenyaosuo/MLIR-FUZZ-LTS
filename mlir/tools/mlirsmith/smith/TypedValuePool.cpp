...
void TypedValuePool::addTypeValue(const TypeValue &typedValue, std::string op,
  ...
  mlir::Operation* parentOp = typedValue.val.getDefiningOp();
  if (!parentOp) {
    mlir::Value val = typedValue.val;
    parentOp = val.getParentRegion()->getParentOp();
  }
  if (!(parentOp && parentOp->getParentOfType<mlir::ModuleOp>() && !failed(parentOp->getParentOfType<mlir::ModuleOp>().verify()))) {
    return;
  }

  // llvm::outs() << "[TypedValuePool::addTypeValue] typedValue.val: " << typedValue.val << "\n";

  switch (ty) {
  case PoolType::Index: {
    assert(mlir::isa<IndexType>(typedValue.type));
  }
  case PoolType::IntOrFloat: {
    assert(typedValue.type.isIntOrFloat());
  }
  case PoolType::Vector: {
    assert(mlir::isa<VectorType>(typedValue.type));
  }
  case PoolType::StaticShapedMemref: {
    assert(mlir::isa<MemRefType>(typedValue.type) &&
           mlir::dyn_cast<MemRefType>(typedValue.type).hasStaticShape());
  }
  case PoolType::DynamicShapedMemref: {
    assert(mlir::isa<MemRefType>(typedValue.type) &&
           mlir::dyn_cast<MemRefType>(typedValue.type).hasRank());
  }
  case PoolType::StaticShapedTensor: {
    assert(mlir::isa<TensorType>(typedValue.type) &&
           mlir::dyn_cast<TensorType>(typedValue.type).hasStaticShape());
  }
  case PoolType::DynamicShapedTensor: {
    assert(mlir::isa<TensorType>(typedValue.type) &&
           mlir::dyn_cast<TensorType>(typedValue.type).hasRank());
  }
...
void TypedValuePool::addElement(const TypeValue &element, std::string op = "") {
  // llvm::outs() << "[TypedValuePool::addElement] element.val: " << element.val << "\n";
  assert(element.val);
...
TypedValuePool::getCandidatesFromIndex(OpBuilder &builder, Location loc) {
  if (indexPool.empty()) {
    indexPool.push_back(
        TypeValue(builder.getIndexType(), arith::ConstantIndexOp::create(builder, loc, 1)));
...
TypeValue TypedValuePool::generateInteger(OpBuilder &builder, Location loc,
  ...
  // auto res = arith::ConstantIntOp::create(builder, loc, UR(2), type);
...
TypeValue TypedValuePool::generateFloat(OpBuilder &builder, Location loc,
  ...
  // auto res = arith::ConstantFloatOp::create(builder, loc, one, type);
  auto tVal = TypeValue(type, res);
  addIntOrFloat(tVal);
  return tVal;
}
...
TypeValue TypedValuePool::generateStaticShapedTensor(OpBuilder &builder,
  ...
  auto t = tensor::EmptyOp::create(builder, loc, shapedType.getShape(), shapedType.getElementType());
  // auto t = tensor::EmptyOp::create(builder, loc, shapedType.getShape(), shapedType.getElementType());
