...
OpGenerator funcGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto funcOp = func::FuncOp::create(builder, loc, funcName, mlir::dyn_cast<FunctionType>(funcType));
...
OpGenerator callGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    return func::CallOp::create(builder, loc, callee, ValueRange(arguments)).getOperation();
  };
}