  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    for (int i = 0; i < eleConstantSize; i++) {
      ...
      if (iType) {
        ...
        // Value cons = arith::ConstantOp::create(builder, loc, builder.getIntegerAttr(elementType, val));
        Value cons = arith::ConstantOp::create(builder, loc, builder.getIntegerAttr(elementType, val));
      ...
      if (fType) {
        ...
        // Value cons = arith::ConstantOp::create(builder, loc, builder.getFloatAttr(elementType, valf));
        Value cons = arith::ConstantOp::create(builder, loc, builder.getFloatAttr(elementType, valf));
