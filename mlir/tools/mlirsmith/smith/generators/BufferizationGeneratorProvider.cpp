...
OpGenerator bufferizationAllocTensorGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto val = bufferization::AllocTensorOp::create(builder, loc, type, ValueRange(dyDimVals));
    // auto val = bufferization::AllocTensorOp::create(builder, loc, type, ValueRange(dyDimVals));
    if (isDynamic) {
...
OpGenerator bufferizationCloneGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto op = bufferization::CloneOp::create(builder, loc, mem.val);
    // auto op = bufferization::CloneOp::create(builder, loc, mem.val);
...
OpGenerator bufferizationDeallocGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    return bufferization::DeallocTensorOp::create(builder, loc, tensor.val).getOperation();
    // return bufferization::DeallocTensorOp::create(builder, loc, tensor.val).getOperation();
  };
}
...
OpGenerator bufferizationToMemrefGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto shapedTy = mlir::dyn_cast<ShapedType>(tensor.type);
    auto resultMemTy =
        MemRefType::get(shapedTy.getShape(), shapedTy.getElementType());
    // auto op =
    //     bufferization::ToMemrefOp::create(builder, loc, resultMemTy, tensor.val);
    auto op =
        bufferization::ToBufferOp::create(builder, loc, resultMemTy, tensor.val);
...
OpGenerator bufferizationToTensorGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto shapedTy = mlir::dyn_cast<ShapedType>(memref.type);
    auto resTensorTy =
        RankedTensorType::get(shapedTy.getShape(), shapedTy.getElementType());
    
    auto op = bufferization::ToTensorOp::create(builder, loc, resTensorTy, memref.val);
    // auto op = bufferization::ToTensorOp::create(builder, loc, resTensorTy, memref.val);
