...
OpGenerator affineApplyGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto ret = AffineApplyOp::create(builder, loc, map, operands);
...
OpGenerator affineForGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto blockBuilder = [&](OpBuilder &b, Location loc,
      ...
      regionGen.applyAll(b, loc);
      if (hasIterArg) {
        auto retCandidates = searchTypedValueFrom(region.pool, iterArg.type);
        if (retCandidates.empty()) {
          retCandidates.push_back(
              region.pool.generateTypedValue(b, loc, iterArg.type));
        }
        auto ret = sampleTypedValueFrom(retCandidates, "scf.yield");
        AffineYieldOp::create(b, loc, ret.val);
      } else {
        AffineYieldOp::create(b, loc);
      }
    };
    if (hasIterArg) {
      auto res = AffineForOp::create(builder, loc, lowerBound, upperBound, step, llvm::ArrayRef(iterArgVal), blockBuilder);
      ...
      // return builder
      //     .create<AffineForOp>(loc, lowerBound, upperBound, step,
      //     std::nullopt,
      //                          blockBuilder)
      //     .getOperation();
      return AffineForOp::create(builder, loc, lowerBound, upperBound, step, mlir::ValueRange(), blockBuilder)
          .getOperation();
    }
  };
}
...
OpGenerator affineIfGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto ifOp = AffineIfOp::create(builder, loc, resultTypes, set, args,
                                   withElseRegion);
    ...
    auto blockBuilder = [&]() {
      ...
      if (hasResult) {
        ...
        AffineYieldOp::create(builder, loc, ValueRange(ret.val));
      }
    };
...
OpGenerator affineLoadGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    assert(memref.val);
    auto res =
        AffineLoadOp::create(builder, loc, memref.val, ValueRange(indices));
...
OpGenerator affineStoreGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    return AffineStoreOp::create(builder, loc, elem2Store.val, memref.val,
                               ValueRange(indices))
        .getOperation();
  };
}
...
OpGenerator affineMaxGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto index = AffineMaxOp::create(builder, loc, map, mlir::ValueRange(mapOperands));
...
OpGenerator affineMinGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto index = AffineMinOp::create(builder, loc, map, mapOperands);
    if (index->getResultTypes().size() == 1 &&
        mlir::dyn_cast<IndexType>(index->getResultTypes()[0])) {
      auto tVal = TypeValue(builder.getIndexType(), index);
...
OpGenerator affineParallelGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto op =
        AffineParallelOp::create(builder, loc, resultTypes, reductions, ranges);
    ...
    builder.setInsertionPointToStart(op.getBody());
    {
      ...
      if (!resultTypes.empty()) {
        ...
        AffineYieldOp::create(builder, loc, ValueRange(rets));
      }
    }
    builder.restoreInsertionPoint(point);
...
//     auto op = AffinePrefetchOp::create(builder, loc, memref.val, indices,
//     UR(2),
//                                                UR(4), UR(2));
//     return op.getOperation();
//   };
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto idxCandidates = parent.pool.searchCandidatesFrom(
        {PoolType::Index}, [](TypeValue tval) {
          return tval.val.getDefiningOp() && tval.val.getParentRegion() && tval.val.getParentRegion()->getParentOp() && isValidDim(tval.val); 
        });
    ...
    auto op = AffineVectorLoadOp::create(builder, loc, vecTy, memref.val,
                                         ValueRange(indices));
...
OpGenerator affineVectorStoreGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    // auto op = AffineVectorStoreOp::create(builder, 
    //     loc, vec2Store.val, memref.val, ValueRange(indices));
    auto op = AffineVectorStoreOp::create(builder, loc, vec2Store.val,
                                          memref.val, ValueRange(indices));
    return op.getOperation();
  };
}