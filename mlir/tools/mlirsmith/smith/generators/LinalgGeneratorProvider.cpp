  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto op = linalg::GenericOp::create(
        ...
        [&](OpBuilder &b, Location loc, ValueRange args) {
          ...
          auto region =
              OpRegion("linalg.generic", parent.depth + 1, parent.cur_child);
    ...
    for (auto res : op->getResults()) {
      ...
      parent.pool.addTypeValue(resV, "linalg.generic");
    }
...
OpGenerator linalgBroadCastGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    long long useMem = UR(2);

    std::vector<TypeValue> inputCandidates;
    if (useMem) {
      inputCandidates = region.pool.searchCandidatesFrom(
    ...
    std::vector<TypeValue> initCandidates;
    if (useMem) {
      initCandidates = region.pool.searchCandidatesFrom(
          {PoolType::StaticShapedMemref, PoolType::DynamicShapedMemref},
          typeEquivalentFilter(initTy));
    } else {
...
OpGenerator linalgTransposeGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    std::vector<TypeValue> fromCandidates;
    if (useMem) {
      fromCandidates = parent.pool.searchCandidatesFrom(
          {PoolType::StaticShapedMemref, PoolType::DynamicShapedMemref},
          [](TypeValue tval) {
    ...
    std::vector<TypeValue> initCandidates;
    if (useMem) {
      initCandidates = parent.pool.searchCandidatesFrom(
          {PoolType::StaticShapedMemref, PoolType::DynamicShapedMemref},
          typeEquivalentFilter(initTy));
    } else {
      initCandidates = parent.pool.searchCandidatesFrom(
...
OpGenerator linalgCopyGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    SmallVector<NamedAttribute> attributes;
    auto op = linalg::CopyOp::create(
        builder, loc, TypeRange({out.val.getType()}), ValueRange({in.val}),
...
OpGenerator linalgMapGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto val = linalg::MapOp::create(builder, loc, TypeRange(initType),
...
OpGenerator linalgReduceGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    std::vector<TypeValue> inputCandidates;
    if (useMem) {
      inputCandidates = typedValuePool.searchCandidatesFrom(
          {PoolType::StaticShapedMemref, PoolType::DynamicShapedMemref},
          [&](TypeValue typeValue) {
    ...
    if (inputCandidates.empty()) {
      ...
      // auto shapedTy = mlir::dyn_cast<ShapedType>(RankedTensorType::get(shape,
      // randomIntOrFloatType(builder.getContext()))); Type ty =
      // randomMemrefOrRankedTensorType(shapedTy);
      Type ty;
      if (useMem) {
        ty = MemRefType::get(shape, randomIntOrFloatType(builder.getContext()));
      } else {
        ty = RankedTensorType::get(shape,
...
OpGenerator linalgDotGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    if (operand0Candidates.empty()) {
      ...
      SmallVector<int64_t> shape;
      shape.push_back(dimPool[UR(dimPool.size())]);
      // auto shapeTy = mlir::dyn_cast<ShapedType>(RankedTensorType::get(shape,
      // randomIntOrFloatType(builder.getContext())));
    ...
    auto operand1Candidates = region.pool.searchCandidatesFrom(
        // {PoolType::StaticShapedTensor, PoolType::StaticShapedMemref},
        {PoolType::StaticShapedTensor}, typeEquivalentFilter(operand1Ty));
...
OpGenerator linalgMatMulGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto res =
        linalg::MatmulOp::create(builder, loc, TypeRange({resTy}),
