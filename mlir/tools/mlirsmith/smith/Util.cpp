...
Value getConstant(Location loc, OpBuilder &b, Type type, long long val) {
  if (mlir::dyn_cast<IntegerType>(type)) {
    ...
    return arith::ConstantOp::create(b, loc, b.getFloatAttr(type, static_cast<double>(val)));
    // return arith::ConstantOp::create(b, loc, b.getFloatAttr(type, static_cast<double>(val)));
  } else {
    if (conf.debug) {
...
TypeValue createNewValue(OpBuilder &builder, Location loc, Type type) {
  if (mlir::dyn_cast<IntegerType>(type)) {
    ...
    auto t = tensor::EmptyOp::create(builder, loc, shapeTy.getShape(), shapeTy.getElementType());
    // auto t = tensor::EmptyOp::create(builder, loc, shapeTy.getShape(), shapeTy.getElementType());
