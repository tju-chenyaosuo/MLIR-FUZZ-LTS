...
void collectAllOperations(mlir::Operation *topOp,
                          std::vector<mlir::Operation *> *ops) {
  for (Region &region : topOp->getRegions()) {
    for (Block &block : region.getBlocks()) {
      for (Operation &op : block.getOperations()) {
        ops->push_back(&op);
        collectAllOperations(&op, ops);
      }
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  op->walk([](func::FuncOp funcOp) {
    ...
    if (numArgs == 0) {
      ...
      std::vector<mlir::Operation *> ops = std::vector<mlir::Operation *>();
      collectAllOperations(funcOp.getOperation(), &ops);

      for (int64_t i = 0; i < ops.size(); ++i) {
        mlir::Operation *o = ops[i];
        mlir::OpBuilder builder(o->getBlock(), o->getBlock()->begin());
        builder.setInsertionPoint(o);
        mlir::Location loc = o->getLoc();
        mlir::Value cons =
            builder
                .create<arith::ConstantOp>(
                    loc, builder.getIntegerAttr(builder.getI64Type(), i))
                .getOperation()
                ->getResult(0);
        vector::PrintOp::create(builder, loc, cons);
      }
    }
  });
