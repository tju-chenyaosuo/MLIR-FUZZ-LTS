...
OpGenerator vectorConstantMaskGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    // }
    // auto arrAttr = DenseI64ArrayAttr::get(builder.getContext(), maskDimSizes);

    SmallVector<long> maskDimSizes;
    for (auto dimSize : shape) {
      maskDimSizes.push_back(rollIdx(dimSize) + 1);
    }
    auto arrAttr = DenseI64ArrayAttr::get(builder.getContext(), maskDimSizes);

    mlir::Value val =
        vector::ConstantMaskOp::create(builder, loc, vectorTy, arrAttr);
    mb->add2Pool(val);
    return true;
  };
}
...
OpGenerator vectorContractGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    vector::ContractionOp::create(builder, loc, vectorLhs, vectorRhs, accOperand,
                                          indexingMapAttr, iteratorTypesAttr,
                                          kind);
    return true;
  };
}
...
OpGenerator vectorExpandLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value val = vector::ExpandLoadOp::create(builder, 
...
OpGenerator vectorExtractElementGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (vectorTy.getRank() == 0) {
      res = vector::ExtractElementOp::create(builder, loc, vector);
    } else {
      ...
      res = vector::ExtractElementOp::create(builder, loc, vector, position);
    }
    mb->add2Pool(res);
    return true;
  };
...
OpGenerator vectorExtractStridedSliceGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = vector::ExtractStridedSliceOp::create(builder, 
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (lhsCandidates.empty()) {
      lhsCandidates.push_back(generateVector(
          builder, loc,
          randomVectorType(FloatType::getF32(builder.getContext())), mb));
    }
...
OpGenerator vectorFlatTransposeGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value val = vector::FlatTransposeOp::create(builder, 
...
OpGenerator vectorGatherGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = vector::GatherOp::create(builder, 
...
OpGenerator vectorInsertElementGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (vectorTy.getRank() == 0) {
      return vector::InsertElementOp::create(builder, loc, source, vector);
    } else {
      std::vector<mlir::Value> idxCandidates = std::vector<mlir::Value>();
      mb->search(index_filter, &idxCandidates);
      if (idxCandidates.empty()) {
      ...
      auto position = idxCandidates[rollIdx(idxCandidates.size())];
      vector::InsertElementOp::create(builder, loc, source, vector, position);
    }
    return true;
  };
}
...
OpGenerator vectorMaskGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (true) {
      ...
      auto kind = kinds[rollIdx(kinds.size())];
      maskableOp = builder
                       .create<vector::MultiDimReductionOp>(loc, operand, acc,
                                                            reductionMask, kind)
                       .getOperation();
    }
...
OpGenerator vectorMaskedLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = vector::MaskedLoadOp::create(builder, 
...
OpGenerator vectorMatrixMultiplyGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value val = vector::MatmulOp::create(builder, 
        loc, lhs, rhs, lhsVTy.getDimSize(0) / reducedDimSize, reducedDimSize,
        rhsVTy.getDimSize(0) / reducedDimSize);
    mb->add2Pool(val);
    return true;
  };
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    std::vector<vector::CombiningKind> kinds =
        mlir::dyn_cast<IntegerType>(vTy.getElementType()) ? intKinds
                                                          : floatKinds;
    auto kind = kinds[rollIdx(kinds.size())];
    mlir::Value val = vector::MultiDimReductionOp::create(builder, 
...
//     auto lhs = lhsCandidates[rollIdx(lhsCandidates.size())];
//     auto lhsElemTy =
//     mlir::dyn_cast<VectorType>(lhs.getType()).getElementType(); std::string
//     lhsTyStr = getValueTypeStr(lhs); std::vector<mlir::Value> rhsCandidates =
//     std::vector<mlir::Value>(); if (mb->valuePool.pool.count(lhsTyStr)) {
...
//     SmallVector<int64_t> accShape;
//     accShape.push_back(mlir::dyn_cast<VectorType>(lhs.getType()).getDimSize(0));
//     accShape.push_back(mlir::dyn_cast<VectorType>(rhs.getType()).getDimSize(0));
...
OpGenerator vectorScanGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    std::vector<vector::CombiningKind> kinds =
        mlir::dyn_cast<IntegerType>(srcVTy.getElementType()) ? intKinds : floatKinds;
    auto kind = kinds[rollIdx(kinds.size())];
...
OpGenerator vectorScatterGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    vector::ScatterOp::create(builder, loc, base, ValueRange(indices), indexVec,
                                      mask, vecToStore);
    return true;
  };
}
...
OpGenerator vectorSplatGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = vector::SplatOp::create(builder, loc, input, vTy);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator vectorTransferReadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto vecTy = VectorType::get(
        vectorShape, mlir::dyn_cast<ShapedType>(src.getType()).getElementType());
    ...
    if (rollIdx(2)) {
      vector::TransferReadOp::create(builder, loc, vecTy, src, indices,
...
OpGenerator vectorTransferWriteGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto destElemTy = mlir::dyn_cast<ShapedType>(dest.getType()).getElementType();
    ...
    auto vecTy = VectorType::get(
        vectorShape, mlir::dyn_cast<ShapedType>(dest.getType()).getElementType());
    std::vector<mlir::Value> vectorCandidates = std::vector<mlir::Value>();
    ...
    vector::TransferWriteOp::create(builder, 
...
OpGenerator vectorWarpExecuteOnLane0Op() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto op = vector::WarpExecuteOnLane0Op::create(builder, loc, TypeRange({}),
                                                           laneId, warpSize);
