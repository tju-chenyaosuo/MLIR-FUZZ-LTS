...
OpGenerator tensorEmptyGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    if (rollIdx(2)) {
      ...
      mlir::Value res = tensor::EmptyOp::create(builder, 
...
OpGenerator tensorExpandShapeGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::ExpandShapeOp::create(builder, 
        loc, resTy, source, reassociation);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator tensorFromElementsGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::FromElementsOp::create(builder, 
        loc, tensorTy, ValueRange(elements));
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator tensorGenerateGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::GenerateOp::create(builder, 
...
OpGenerator tensorScatterGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res = tensor::ScatterOp::create(builder, 
  ...
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mb->search(
        [](std::string s, mlir::Value v) -> bool {
          if (!static_tensor_filter(s, v))
            return false;
          return mlir::dyn_cast<RankedTensorType>(v.getType()).getRank() % 2 == 0;
        },
        &candidates);
    if (candidates.empty()) {
      SmallVector<int64_t> shape;
