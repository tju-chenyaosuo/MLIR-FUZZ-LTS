...
OpGenerator affineForGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (hasIterArg) {
      auto res = mlir::affine::AffineForOp::create(builder, 
          loc, lowerBound, upperBound, step, llvm::ArrayRef(iterArgVal),
          blockBuilder);
    } else {
      auto value = mlir::affine::AffineForOp::create(builder, 
          loc, lowerBound, upperBound, step, std::nullopt, blockBuilder);
    }
    return true;
  };
}
...
OpGenerator affineLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    // Generate operation
    assert(memref);
    mlir::Value res = mlir::affine::AffineLoadOp::create(builder, 
...
OpGenerator affineParallelGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    auto op = mlir::affine::AffineParallelOp::create(builder, 
        loc, resultTypes, reductions, ranges);
...
OpGenerator affinePrefetchGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    auto memref = memCandidates[rollIdx(memCandidates.size())];
    SmallVector<Value> indices;
    mlir::affine::AffinePrefetchOp::create(builder, 
        loc, memref, indices, rollIdx(2), rollIdx(4), rollIdx(2),
...
OpGenerator affineVectorLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    mlir::Value op = mlir::affine::AffineVectorLoadOp::create(builder, 
        loc, vecTy, memref, ValueRange(indices));
...
OpGenerator affineVectorStoreGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *parentMb) -> bool {
    ...
    parentMb->search(
        [&](std::string s, mlir::Value v) -> bool {
          ...
          if (auto rankedMemRefType = mlir::dyn_cast<MemRefType>(valueType))
            cond2 = v.getType().dyn_cast<ShapedType>().getElementType() ==
    ...
    auto op = mlir::affine::AffineVectorStoreOp::create(builder, 
        loc, vec2Store, memref, ValueRange(indices));
    return true;
  };
}
