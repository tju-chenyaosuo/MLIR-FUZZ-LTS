...
generateInteger(OpBuilder &builder, Location loc, IntegerType type) {
  mlir::Value res = arith::ConstantIntOp::create(builder, loc, rollIdx(2), type);
  return res;
}
...
generateFloat(OpBuilder &builder, Location loc, FloatType type) {
  ...
  mlir::Value res = arith::ConstantFloatOp::create(builder, loc, one, type);
  return res;
}
...
mlir::Value createNewValue(OpBuilder &builder, Location loc, Type type) {
  if (type.dyn_cast<IntegerType>()) {
    ...
    if (ty.hasStaticShape()) {
      auto m = memref::AllocOp::create(builder, loc, type.dyn_cast<MemRefType>());
      return m;
    } else {
      assert(false);
    }
