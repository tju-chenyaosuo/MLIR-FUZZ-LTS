...
OpGenerator memrefLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    SmallVector<Value, 8> loadIndices =
        randomIndicesForShapedType(t, builder, loc);
...
OpGenerator memrefStoreGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (mb->valuePool.pool.count(elemTypeStr)) {
      candidateValueToStore.insert(candidateValueToStore.begin(),
    ...
    auto tValToStore =
        candidateValueToStore[rollIdx(candidateValueToStore.size())];
    auto storeIndices = randomIndicesForShapedType(t, builder, loc);
    memref::StoreOp::create(builder, loc, tValToStore, alloc, storeIndices);
    return true;
  };
}
...
OpGenerator atomicRMWGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    std::string elemTypeStr = getValueTypeStr(elemType);
    if (mb->valuePool.pool.count(elemTypeStr)) {
      elemCandidates.insert(elemCandidates.end(),
                            mb->valuePool.pool[elemTypeStr].begin(),
                            mb->valuePool.pool[elemTypeStr].end());
    }
...
OpGenerator memrefCopyGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (srcCandidates.empty()) {
      srcCandidates.push_back(generateRankedMemref(
          builder, loc, randomRankedMemrefType(builder.getContext()), mb));
    }
...
OpGenerator tensorStoreGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto srcShape = mlir::dyn_cast<ShapedType>(tensor.getType());
    ...
    if (mb->valuePool.pool.count(destMemTyStr)) {
      destSrcCandidates.insert(destSrcCandidates.begin(),
    ...
    if (destSrcCandidates.empty()) {
      destSrcCandidates.push_back(
          generateRankedMemref(builder, loc, destMemTy, mb));
    }
    auto destMem = destSrcCandidates[rollIdx(destSrcCandidates.size())];
    bufferization::MaterializeInDestinationOp::create(builder, loc, tensor, destMem);
    return true;
  };
}
...
OpGenerator genericAtomicRMWGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto op = memref::GenericAtomicRMWOp::create(builder, loc, memref,
                                                         ValueRange(indices));
    ...
    if (childMb->valuePool.pool.count(elemTypeStr)) {
      yieldCandidates.insert(yieldCandidates.end(),
    ...
    op.getBodyRegion().push_back(entry);
    {
      ...
      if (!results.empty()) {
        ...
        if (mb->valuePool.pool.count(retTyStr)) {
          retCandidates.insert(retCandidates.begin(),
