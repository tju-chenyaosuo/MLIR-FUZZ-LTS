...
Value getConstant(Location loc, OpBuilder &b, Type type, long long val) {
  if (mlir::dyn_cast<IntegerType>(type)) {
    return arith::ConstantOp::create(b, loc, b.getIntegerAttr(type, val));
...
searchShapedInputFrom(ShapedType type, const std::vector<TypeValue> &pool) {
  ...
  for (auto tensor : pool) {
    ShapedType t = mlir::dyn_cast<ShapedType>(tensor.type);
...
searchIntegerCandidatesFrom(std::vector<TypeValue> pool) {
  ...
  for (auto elem : pool) {
    RankedTensorType t = mlir::dyn_cast<RankedTensorType>(elem.type);
    if (t &&
        mlir::dyn_cast<IntegerType>(mlir::dyn_cast<ShapedType>(t).getElementType())) {
      res.push_back(elem);
    } else if (mlir::dyn_cast<IntegerType>(elem.type)) {
      res.push_back(elem);
    }
  }
  return res;
...
std::vector<TypeValue> searchFloatCandidatesFrom(std::vector<TypeValue> pool) {
  ...
  for (auto elem : pool) {
    RankedTensorType t = mlir::dyn_cast<RankedTensorType>(elem.type);
    if (t && mlir::dyn_cast<FloatType>(mlir::dyn_cast<ShapedType>(t).getElementType())) {
      res.push_back(elem);
    } else if (mlir::dyn_cast<FloatType>(elem.type)) {
      res.push_back(elem);
    }
  }
  return res;
...
std::vector<mlir::Type> getSupportedFloatTypes(MLIRContext *ctx) {
  std::vector<mlir::Type> res = { mlir::Float16Type::get(ctx), mlir::Float32Type::get(ctx)
    // FloatType::getF16(ctx), FloatType::getF32(ctx)
...
std::vector<TypeValue> search2DTensorsFrom(const std::vector<TypeValue> pool) {
  std::vector<TypeValue> res;
  for (auto tensor : pool) {
    if (mlir::dyn_cast<RankedTensorType>(tensor.type) &&
        mlir::dyn_cast<ShapedType>(tensor.type).getShape().size() == 2) {
...
std::vector<TypeValue> search2thTensorsFor(const std::vector<TypeValue> pool,
  ...
  for (auto tensor : pool) {
    if (mlir::dyn_cast<RankedTensorType>(tensor.type) &&
        mlir::dyn_cast<ShapedType>(tensor.type).getElementType() ==
            shapedType.getElementType() &&
        mlir::dyn_cast<ShapedType>(tensor.type).getShape().size() == 2 &&
        mlir::dyn_cast<ShapedType>(tensor.type).getShape()[0] ==
...
TypeValue createNewValue(OpBuilder &builder, Location loc, Type type) {
  if (mlir::dyn_cast<IntegerType>(type)) {
    ...
    if (ty.hasStaticShape()) {
      auto m =
          memref::AllocOp::create(builder, loc, mlir::dyn_cast<MemRefType>(type));
    ...
    auto t = tensor::EmptyOp::create(builder, loc, shapeTy.getShape(),
