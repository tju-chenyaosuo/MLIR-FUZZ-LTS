...
OpGenerator absIGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    //     });
    auto operandCandidates = region.pool.searchCandidatesFrom(
        {PoolType::IntOrFloat}, [&](TypeValue tval) {
    ...
    auto tVal = TypeValue(operand.type, value);
    region.pool.addTypeValue(tVal, "math.absi");
    return value.getOperation();
  };
}
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto operandCandidates = typedValuePool.searchCandidatesFrom(
        {PoolType::IntOrFloat, PoolType::DynamicShapedTensor,
         PoolType::StaticShapedTensor},
        [&](TypeValue tval) {
          return mlir::isa<FloatType>(tval.type) || (mlir::isa<ShapedType>(tval.type) &&
                                                mlir::isa<FloatType>(mlir::dyn_cast<ShapedType>(tval.type).getElementType()));
        });
    if (operandCandidates.empty()) {
      operandCandidates.push_back(
          typedValuePool.generateFloat(builder, loc, builder.getF32Type()));
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto operandCandidates = region.pool.searchCandidatesFrom(
        {PoolType::IntOrFloat}, [&](TypeValue tval) {
          return mlir::isa<IntegerType>(tval.type) ||
                 (mlir::isa<ShapedType>(tval.type) &&
                  mlir::isa<IntegerType>(mlir::dyn_cast<ShapedType>(tval.type).getElementType()));
        });
    if (operandCandidates.empty()) {
      operandCandidates.push_back(
          region.pool.generateInteger(builder, loc, builder.getI32Type()));
    ...
    auto tVal = TypeValue(operand.type, value);
    region.pool.addTypeValue(tVal, "math.ctzl");
    return value.getOperation();
  };
}
...
OpGenerator cttzGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto tVal = TypeValue(operand.type, value);
    region.pool.addTypeValue(tVal, "math.cttz");
    return value.getOperation();
  };
}
...
OpGenerator ctpopGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto tVal = TypeValue(operand.type, value);
    region.pool.addTypeValue(tVal, "math.ctpop");
    return value.getOperation();
  };
}
