  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto blockBuilder = [&](OpBuilder &b, Location loc) {
      ...
      if (!resultTypes.empty()) {
        ...
        scf::YieldOp::create(builder, loc, ret.val);
      } else {
    ...
    scf::IfOp ifOp;
    if (hasElse) {
      ifOp = scf::IfOp::create(builder, loc, condition.val, blockBuilder,
                               blockBuilder);
    } else {
      ifOp = scf::IfOp::create(builder, loc, condition.val, blockBuilder);
    }
...
OpGenerator executeRegionGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto op = scf::ExecuteRegionOp::create(builder, loc, resultTypes);
...
OpGenerator scfForGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto res = scf::ForOp::create(builder, loc, lowerBound, upperBound, step,
...
OpGenerator indexSwitchGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto idxSwitchOp = scf::IndexSwitchOp::create(
        builder, loc, TypeRange(resultTypes), arg.val, cases, caseRegionCount);
    auto blockBuilder = [&](OpBuilder &b, Location loc) {
...
OpGenerator scfWhileGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto doBuilder = [&](OpBuilder &b, Location loc, ValueRange args) {
      ...
      // llvm::outs()<<"searching scf.while op return value:
      // "<<retCandidates.size()<<"\n";
      if (retCandidates.empty()) {
        // llvm::outs()<<"scf.while op return value is empty.\n";
        retCandidates.push_back(
    ...
    auto condBuilder = [&](OpBuilder &b, Location loc, ValueRange args) {
      ...
      // force generate the false while loop.(never execute)
      mlir::Value cond = index::BoolConstantOp::create(builder, loc, 0);
      ...
      scf::ConditionOp::create(builder, loc, cond, ValueRange(condOperand.val));
    };
    return scf::WhileOp::create(builder, loc, TypeRange(resultType),
                                ValueRange(operand0.val), condBuilder,
                                doBuilder)
        .getOperation();
  };
}
...
//    auto op = scf::ForeachThreadOp::create(builder,
...
//          gen.apply(b, loc, 16);
//          scf::PerformConcurrentlyOp::create(b, loc);
//        });
//
//    return op.getOperation();
//  };
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto bodyBuilder = [&](OpBuilder &b, Location loc, ValueRange ivs,
      ...
      if (hasOperand) {
        ...
        //       scfReduceRegion.pool.addTypeValue(TypeValue(lhs.getType(),
        //       lhs),
        //                                         "arg()");
        //       scfReduceRegion.pool.addTypeValue(TypeValue(lhs.getType(),
        //       lhs),
        ...
        //                                             "scf.reduce.return");
        //       scf::ReduceReturnOp::create(builder1, loc, reduceRet.val);
        //     });
        scf::ReduceOp::create(b, loc, reduceArg.val);
      }
    };

    return scf::ParallelOp::create(builder, loc, ValueRange(lowerBounds),
