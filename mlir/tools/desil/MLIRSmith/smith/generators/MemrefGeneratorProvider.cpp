...
OpGenerator memrefStoreGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto storeIndices = typedValuePool.randomIndicesForShapedType(
        mlir::dyn_cast<ShapedType>(typedAlloc.type), builder, loc);
    return memref::StoreOp::create(builder, loc, tValToStore.val, alloc,
                                   storeIndices)
        .getOperation();
  };
}
...
OpGenerator atomicRMWGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto res =
        memref::AtomicRMWOp::create(builder, loc, elemType, kind, elem.val,
                                    memref.val, ValueRange(loadIndices));
...
OpGenerator assumeAlignmentGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    if (memCandidates.empty()) {
      ...
      auto tVal =
          TypeValue(memType, initMemref(builder, loc,
                                        mlir::dyn_cast<MemRefType>(memType)));
...
OpGenerator tensorStoreGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto srcShape = mlir::dyn_cast<ShapedType>(tensor.type);
    ...
    return bufferization::MaterializeInDestinationOp::create(
               builder, loc, tensor.val, destMem.val)
        .getOperation();
  };
}
...
OpGenerator memrefCastGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    if (memrefCandidates.empty()) {
      memrefCandidates.push_back(typedValuePool.generateRankedMemref(
          builder, loc, randomRankedMemrefType(builder.getContext())));
    }
