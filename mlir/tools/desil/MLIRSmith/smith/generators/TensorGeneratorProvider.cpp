  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto res = tensor::GenerateOp::create(
        ...
        [&](OpBuilder &b, Location loc, ValueRange args) {
          ...
          tensor::YieldOp::create(builder, loc, ret.val);
        });
...
OpGenerator tensorPackGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    //                             innerTiles, std::optional<Value>(pad.val),
    //                             outerDimsPos)
    //     .getOperation();
    return mlir::linalg::PackOp::create(
               builder, loc, src.val, dest.val, innerDimsPos, innerTiles,
               std::optional<Value>(pad.val), outerDimsPos)
        .getOperation();
  };
}
...
OpGenerator tensorUnpackGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    //     .getOperation();
    return mlir::linalg::UnPackOp::create(builder, loc, source.val, dest.val,
