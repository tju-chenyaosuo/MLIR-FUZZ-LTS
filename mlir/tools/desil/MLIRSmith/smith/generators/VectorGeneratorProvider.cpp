...
OpGenerator vectorConstantMaskGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    for (auto dimSize : shape) {
      maskDimSizes.push_back(UR(dimSize) + 1);
    }
    auto arrAttr = DenseI64ArrayAttr::get(builder.getContext(), maskDimSizes);

    auto val = vector::ConstantMaskOp::create(builder, loc, vectorTy, arrAttr);
...
OpGenerator vectorExtractGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto res =
        vector::ExtractOp::create(builder, loc, vectorOperand.val, arrAttr);
...
OpGenerator vectorFlatTransposeGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    // auto val = vector::FlatTransposeOp::create(builder, loc, vectorTy,
...
OpGenerator vectorLoadGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    auto memrefCandidates = region.pool.searchCandidatesFrom(
        {PoolType::StaticShapedMemref, PoolType::DynamicShapedMemref},
        [](TypeValue tval) {
          return mlir::dyn_cast<ShapedType>(tval.type).getRank() ==
                 1; // in the -convert-vector-to-llvm pass defining file, only
                    // vector with one rank can be lowered
        });
    if (memrefCandidates.empty()) {
      std::vector<int64_t>
          shape; // in the -convert-vector-to-llvm pass defining file, only
                 // vector with one rank can be lowered
      if (UR(2)) {
        shape.push_back(dimPool[UR(dimPool.size())]);
      } else {
      ...
      memrefCandidates.push_back(
          region.pool.generateRankedMemref(builder, loc, memTy));
      // memrefCandidates.push_back(region.pool.generateRankedMemref(
      //     builder, loc, randomRankedMemrefType(builder.getContext())));
    }
    ...
    // if(memrefTy.getDimSize(0) > 0)
    //   shape.push_back(memrefTy.getDimSize(0));
    // else
    //   shape.push_back(ShapedType::kDynamic);
    auto vectorTy = random1DVectorType(
        memrefTy.getElementType()); // in the -convert-vector-to-llvm pass
                                    // defining file, only vector with one rank
                                    // can be lowered
...
OpGenerator vectorMatrixMultiplyGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    // auto val = vector::MatmulOp::create(
    ...
    auto val = mlir::LLVM::MatrixMultiplyOp::create(
        builder, loc, resVTy, lhs.val, rhs.val,
        lhsVTy.getDimSize(0) / reducedDimSize, reducedDimSize,
        rhsVTy.getDimSize(0) / reducedDimSize);
...
OpGenerator vectorScatterGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    // return vector::ScatterOp::create(builder, loc, base.val,
    //                                  ValueRange(indices), indexVec.val,
    //                                  mask.val, vecToStore.val)
    //     .getOperation();
    return vector::ScatterOp::create(builder, loc, /*resultType=*/nullptr,
                                     base.val, ValueRange(indices),
                                     indexVec.val, mask.val, vecToStore.val).getOperation();
  };
}
...
OpGenerator vectorSplatGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    // auto res = vector::SplatOp::create(builder, loc, input.val, vTy);
...
OpGenerator vectorTransferReadGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    // if (UR(2)) {
    //   return vector::TransferReadOp::create(builder, loc, vecTy, src.val,
...
OpGenerator vectorTransferWriteGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    return vector::TransferWriteOp::create(builder, loc, vector.val, dest.val,
                                           indices, permutationMapAttr, nullptr,
                                           nullptr)
        .getOperation();
  };
}
