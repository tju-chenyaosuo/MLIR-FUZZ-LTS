...
OpGenerator bufferizationAllocTensorGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto val = bufferization::AllocTensorOp::create(builder, 
...
OpGenerator bufferizationDeallocGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    return bufferization::DeallocTensorOp::create(builder, loc, tensor.val)
        .getOperation();
  };
}
...
OpGenerator bufferizationToTensorGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto op =
        bufferization::ToTensorOp::create(builder, loc, resTensorTy, memref.val);
