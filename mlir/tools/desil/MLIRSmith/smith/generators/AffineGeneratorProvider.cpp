...
OpGenerator affineApplyGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    mlir::AffineMap map = mapCandidates[UR(mapCandidates.size())];
    ...
    auto ret = mlir::affine::AffineApplyOp::create(builder, loc, map, operands);
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto blockBuilder = [&](OpBuilder &b, Location loc,
      ...
      if (hasIterArg) {
        ...
        mlir::affine::AffineYieldOp::create(builder, loc, ret.val);
      } else {
    ...
    if (hasIterArg) {
      ...
      return mlir::affine::AffineForOp::create(builder, loc, lowerBound, upperBound, step, mlir::ValueRange(), blockBuilder).getOperation();
    }
  };
}
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto blockBuilder = [&]() {
      ...
      if (hasResult) {
        ...
        mlir::affine::AffineYieldOp::create(builder, loc, ValueRange(ret.val));
      }
    };
...
OpGenerator affineLoadGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    assert(memref.val);
    auto res = mlir::affine::AffineLoadOp::create(builder, loc, memref.val,
                                                          ValueRange(indices));
...
OpGenerator affineStoreGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    return mlir::affine::AffineStoreOp::create(builder, loc, elem2Store.val, memref.val, ValueRange(indices))
        .getOperation();
  };
}
...
OpGenerator affineMaxGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    if (index->getResultTypes().size() == 1 &&
        mlir::dyn_cast<IndexType>(index->getResultTypes()[0])) {
...
OpGenerator affineParallelGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto point = builder.saveInsertionPoint();
    auto &loopBody = op.getLoopRegions().front();
...
OpGenerator affinePrefetchGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    auto mapCandidates = parent.pool.affineMapPool;
    ...
    mlir::AffineMap map = mapCandidates[UR(mapCandidates.size())];
    auto op = mlir::affine::AffinePrefetchOp::create(builder, 
        loc, memref.val, indices, UR(2), UR(4), UR(2), map);
    return op.getOperation();
  };
}
...
OpGenerator affineVectorLoadGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto idxCandidates =
        parent.pool.searchCandidatesFrom({PoolType::Index}, [](TypeValue tval) {
          return mlir::affine::isValidDim(tval.val);
        });
    assert(!idxCandidates.empty());
    SmallVector<Value> indices;
    auto memTy = mlir::dyn_cast<MemRefType>(memref.type);
    ...
    auto op = mlir::affine::AffineVectorLoadOp::create(builder, 
        loc, vecTy, memref.val, ValueRange(indices));
...
OpGenerator affineVectorStoreGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &parent) {
    ...
    auto vecTy = mlir::dyn_cast<VectorType>(vec2Store.type);

    auto memCandidates = parent.pool.searchCandidatesFrom(
        {PoolType::StaticShapedMemref, PoolType::DynamicShapedMemref},
        [&](TypeValue tval) {
          return mlir::dyn_cast<ShapedType>(tval.type).getElementType() ==
    ...
    assert(mlir::affine::isValidDim(parent.pool.constantIndices[0]));
    assert(!idxCandidates.empty());
    SmallVector<Value> indices;
    auto memTy = mlir::dyn_cast<MemRefType>(memref.type);
    for (int i = 0; i < memTy.getRank(); ++i) {
